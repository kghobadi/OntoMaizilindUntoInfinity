...
// Light Main Light
uniform float4 MainLight_47751952PosAndRange;
uniform float4 MainLight_47751952ColorAndIntensity;
uniform float3 MainLight_47751952Direction;
uniform float MainLight_47751952Penumbra;
uniform int MainLight_47751952ShadowSteps;
// Light Fill Light
uniform float4 FillLight_47751799PosAndRange;
uniform float4 FillLight_47751799ColorAndIntensity;
uniform float3 FillLight_47751799Direction;
uniform float FillLight_47751799Penumbra;
uniform int FillLight_47751799ShadowSteps;

// UNIFORMS AND FUNCTIONS
uniform float x_47750743_ce8993a9_x;
uniform float x_47750743_ce8993a9_y;
uniform float x_47750743_ce8993a9_z;
float object_Box(float3 p , float _INP_x, float _INP_y, float _INP_z) {
    // Generated from Assets/Imported/Raymarching Toolkit/Assets/Snippets/Objects/Box.asset
    float3 d = abs(p)-float3(_INP_x,_INP_y,_INP_z);
    float b = min(max(d.x, max(d.y,d.z)), 0) + length(max(d, 0));
    return b;
}
uniform float x_47749939_6492bb9b_radius;
uniform float x_47750805_6492bb9b_radius;
uniform float x_47751022_6492bb9b_radius;
uniform float x_47750842_6492bb9b_radius;
uniform float x_47750813_6492bb9b_radius;
uniform float x_47750749_6492bb9b_radius;
uniform float x_47749844_6492bb9b_radius;
...
uniform float x_47750968_399aefe0_radius;
uniform float x_47750968_399aefe0_height;
uniform float x_47750962_399aefe0_radius;
uniform float x_47750962_399aefe0_height;
uniform float x_47750995_399aefe0_radius;
uniform float x_47750995_399aefe0_height;
uniform float x_47750034_399aefe0_radius;
uniform float x_47750034_399aefe0_height;
uniform float x_47749817_399aefe0_radius;
uniform float x_47749817_399aefe0_height;
uniform float x_47751921_399aefe0_radius;
uniform float x_47751921_399aefe0_height;
uniform float x_47749968_399aefe0_radius;
uniform float x_47749968_399aefe0_height;
...
// uniforms for bg box
uniform float4x4 _47750743Matrix;
uniform float _47750743MinScale;
// uniforms for Sphere
uniform float4x4 _47749939Matrix;
uniform float _47749939MinScale;
// uniforms for Cylinder
uniform float4x4 _47750968Matrix;
uniform float _47750968MinScale;
// uniforms for Sphere
uniform float4x4 _47750805Matrix;
uniform float _47750805MinScale;
// uniforms for Cylinder
uniform float4x4 _47750962Matrix;
uniform float _47750962MinScale;
// uniforms for Sphere
uniform float4x4 _47751022Matrix;
uniform float _47751022MinScale;
// uniforms for Cylinder
uniform float4x4 _47750995Matrix;
uniform float _47750995MinScale;
// uniforms for Sphere
uniform float4x4 _47750842Matrix;
uniform float _47750842MinScale;
// uniforms for Cylinder
uniform float4x4 _47750034Matrix;
uniform float _47750034MinScale;
// uniforms for Sphere
uniform float4x4 _47750813Matrix;
uniform float _47750813MinScale;
// uniforms for Cylinder
uniform float4x4 _47749817Matrix;
uniform float _47749817MinScale;
// uniforms for Sphere
uniform float4x4 _47750749Matrix;
uniform float _47750749MinScale;
// uniforms for Cylinder
uniform float4x4 _47751921Matrix;
uniform float _47751921MinScale;
// uniforms for Sphere
uniform float4x4 _47749844Matrix;
uniform float _47749844MinScale;
// uniforms for Cylinder
uniform float4x4 _47749968Matrix;
uniform float _47749968MinScale;
uniform float x_47749075_8d0fbb1f_smoothfactor;
...
uniform float x_47751706_cd4562de_n;
uniform float x_47751706_cd4562de_r;
float2 blend_Pillows(float2 a, float2 b , float _INP_n, float _INP_r) {
    // Generated from Assets/Imported/Raymarching Toolkit/Assets/Snippets/Blends/Pillows.asset
    float v = min(a, b);
    if ((a.x < _INP_r) && (b.x < _INP_r)) {
      float2 p = float2(a.x, b.x);
      float columnradius = _INP_r*sqrt(2)/((_INP_n-1)*2+sqrt(2));
      pR45(p);
      p.x -= sqrt(2)/2*_INP_r;
...
uniform float x_47749788_44192f17_intensity;
...
uniform int x_47750063_d6c1f870_steps;
uniform float x_47750063_d6c1f870_intensity;
float2 blend_Stairs(float2 a, float2 b , int _INP_steps, float _INP_intensity) {
    // Generated from Assets/Imported/Raymarching Toolkit/Assets/Snippets/Blends/Stairs.asset
    float n = _INP_steps;
    float s = _INP_intensity/n;
    float u = b-_INP_intensity;
    
    float v = min(min(a,b), float2(0.5, 1) * (u + a + abs ((modc(u - a + s, 2 * s)) - s)));
...
float2 blend_Subtract(float2 a, float2 b /*, [object params] */) {
    // Generated from Assets/Imported/Raymarching Toolkit/Assets/Snippets/Blends/Subtract.asset
    return float2(max(-a.x, b.x), b.y);
}
float2 blend_Intersection(float2 a, float2 b /*, [object params] */) {
    // Generated from Assets/Imported/Raymarching Toolkit/Assets/Snippets/Blends/Intersection.asset
    return max(a, b);
    
}
uniform float x_47751028_131749b2_whichObject;
float2 blend_Morph(float2 a, float2 b , float _INP_whichObject) {
    // Generated from Assets/Imported/Raymarching Toolkit/Assets/Snippets/Blends/Morph.asset
    return lerp(a, b, _INP_whichObject);
}
uniform float2 x_47750743_07ee3232_position;
uniform sampler2D x_47750743_07ee3232_gradient;
...
uniform float4 x_47749939_da843a44_color;
uniform float4 x_47750968_da843a44_color;
uniform float4 x_47750805_da843a44_color;
uniform float4 x_47750962_da843a44_color;
uniform float4 x_47751022_da843a44_color;
uniform float4 x_47750995_da843a44_color;
uniform float4 x_47750842_da843a44_color;
uniform float4 x_47750034_da843a44_color;
uniform float4 x_47750813_da843a44_color;
uniform float4 x_47749817_da843a44_color;
uniform float4 x_47750749_da843a44_color;
uniform float4 x_47751921_da843a44_color;
uniform float4 x_47749844_da843a44_color;
uniform float4 x_47749968_da843a44_color;
...
{
    objectID = ceil(nf) / (float)15;
    [branch] if (nf <= 1) {
    //    objectID = 0.06666667;
        return material_Gradient(normal, objPos(_47750743Matrix, p), rayDir, x_47750743_07ee3232_position, x_47750743_07ee3232_gradient);
    }
    else if(nf <= 2) {
    //    objectID = 0.1333333;
        return material_SimpleColor(normal, objPos(_47749939Matrix, p), rayDir, x_47749939_da843a44_color);
    }
    else if(nf <= 3) {
    //    objectID = 0.2;
        return material_SimpleColor(normal, objPos(_47750968Matrix, p), rayDir, x_47750968_da843a44_color);
    }
    else if(nf <= 4) {
    //    objectID = 0.2666667;
        return material_SimpleColor(normal, objPos(_47750805Matrix, p), rayDir, x_47750805_da843a44_color);
    }
    else if(nf <= 5) {
    //    objectID = 0.3333333;
        return material_SimpleColor(normal, objPos(_47750962Matrix, p), rayDir, x_47750962_da843a44_color);
    }
    else if(nf <= 6) {
    //    objectID = 0.4;
        return material_SimpleColor(normal, objPos(_47751022Matrix, p), rayDir, x_47751022_da843a44_color);
    }
    else if(nf <= 7) {
    //    objectID = 0.4666667;
        return material_SimpleColor(normal, objPos(_47750995Matrix, p), rayDir, x_47750995_da843a44_color);
    }
    else if(nf <= 8) {
    //    objectID = 0.5333334;
        return material_SimpleColor(normal, objPos(_47750842Matrix, p), rayDir, x_47750842_da843a44_color);
    }
    else if(nf <= 9) {
    //    objectID = 0.6;
        return material_SimpleColor(normal, objPos(_47750034Matrix, p), rayDir, x_47750034_da843a44_color);
    }
    else if(nf <= 10) {
    //    objectID = 0.6666667;
        return material_SimpleColor(normal, objPos(_47750813Matrix, p), rayDir, x_47750813_da843a44_color);
    }
    else if(nf <= 11) {
    //    objectID = 0.7333333;
        return material_SimpleColor(normal, objPos(_47749817Matrix, p), rayDir, x_47749817_da843a44_color);
    }
    else if(nf <= 12) {
    //    objectID = 0.8;
        return material_SimpleColor(normal, objPos(_47750749Matrix, p), rayDir, x_47750749_da843a44_color);
    }
    else if(nf <= 13) {
    //    objectID = 0.8666667;
        return material_SimpleColor(normal, objPos(_47751921Matrix, p), rayDir, x_47751921_da843a44_color);
    }
    else if(nf <= 14) {
    //    objectID = 0.9333333;
        return material_SimpleColor(normal, objPos(_47749844Matrix, p), rayDir, x_47749844_da843a44_color);
    }
    else if(nf <= 15) {
    //    objectID = 1;
        return material_SimpleColor(normal, objPos(_47749968Matrix, p), rayDir, x_47749968_da843a44_color);
    }
        objectID = 0;
...
{
    float _47750743Distance = object_Box(objPos(_47750743Matrix, p), x_47750743_ce8993a9_x, x_47750743_ce8993a9_y, x_47750743_ce8993a9_z) * _47750743MinScale;
    float _47749939Distance = object_Sphere(objPos(_47749939Matrix, p), x_47749939_6492bb9b_radius) * _47749939MinScale;
    float _47750968Distance = object_Cylinder(objPos(_47750968Matrix, p), x_47750968_399aefe0_radius, x_47750968_399aefe0_height) * _47750968MinScale;
    float _47750805Distance = object_Sphere(objPos(_47750805Matrix, p), x_47750805_6492bb9b_radius) * _47750805MinScale;
    float _47750962Distance = object_Cylinder(objPos(_47750962Matrix, p), x_47750962_399aefe0_radius, x_47750962_399aefe0_height) * _47750962MinScale;
    float _47751022Distance = object_Sphere(objPos(_47751022Matrix, p), x_47751022_6492bb9b_radius) * _47751022MinScale;
    float _47750995Distance = object_Cylinder(objPos(_47750995Matrix, p), x_47750995_399aefe0_radius, x_47750995_399aefe0_height) * _47750995MinScale;
    float _47750842Distance = object_Sphere(objPos(_47750842Matrix, p), x_47750842_6492bb9b_radius) * _47750842MinScale;
    float _47750034Distance = object_Cylinder(objPos(_47750034Matrix, p), x_47750034_399aefe0_radius, x_47750034_399aefe0_height) * _47750034MinScale;
    float _47750813Distance = object_Sphere(objPos(_47750813Matrix, p), x_47750813_6492bb9b_radius) * _47750813MinScale;
    float _47749817Distance = object_Cylinder(objPos(_47749817Matrix, p), x_47749817_399aefe0_radius, x_47749817_399aefe0_height) * _47749817MinScale;
    float _47750749Distance = object_Sphere(objPos(_47750749Matrix, p), x_47750749_6492bb9b_radius) * _47750749MinScale;
    float _47751921Distance = object_Cylinder(objPos(_47751921Matrix, p), x_47751921_399aefe0_radius, x_47751921_399aefe0_height) * _47751921MinScale;
    float _47749844Distance = object_Sphere(objPos(_47749844Matrix, p), x_47749844_6492bb9b_radius) * _47749844MinScale;
    float _47749968Distance = object_Cylinder(objPos(_47749968Matrix, p), x_47749968_399aefe0_radius, x_47749968_399aefe0_height) * _47749968MinScale;
    result = opU(opU(opU(opU(opU(opU(opU(float2(_47750743Distance, /*material ID*/0.5), blend_SmoothIntersection(float2(_47749939Distance, /*material ID*/1.5), float2(_47750968Distance, /*material ID*/2.5), x_47749075_8d0fbb1f_smoothfactor)), blend_Pillows(float2(_47750805Distance, /*material ID*/3.5), float2(_47750962Distance, /*material ID*/4.5), x_47751706_cd4562de_n, x_47751706_cd4562de_r)), blend_Smooth(float2(_47751022Distance, /*material ID*/5.5), float2(_47750995Distance, /*material ID*/6.5), x_47749788_44192f17_intensity)), blend_Stairs(float2(_47750842Distance, /*material ID*/7.5), float2(_47750034Distance, /*material ID*/8.5), x_47750063_d6c1f870_steps, x_47750063_d6c1f870_intensity)), blend_Subtract(float2(_47750813Distance, /*material ID*/9.5), float2(_47749817Distance, /*material ID*/10.5))), blend_Intersection(float2(_47750749Distance, /*material ID*/11.5), float2(_47751921Distance, /*material ID*/12.5))), blend_Morph(float2(_47749844Distance, /*material ID*/13.5), float2(_47749968Distance, /*material ID*/14.5), x_47751028_131749b2_whichObject));
    }
	return result;
}
...
{
LightInfo light;
light.posAndRange = MainLight_47751952PosAndRange;
light.colorAndIntensity = MainLight_47751952ColorAndIntensity;
light.direction = MainLight_47751952Direction;
lightValue += getDirectionalLight(input, light)* softshadow(input.pos, -light.direction, INFINITY, MainLight_47751952Penumbra, MainLight_47751952ShadowSteps);
}
{
LightInfo light;
light.posAndRange = FillLight_47751799PosAndRange;
light.colorAndIntensity = FillLight_47751799ColorAndIntensity;
light.direction = FillLight_47751799Direction;
